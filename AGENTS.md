...
- Node: `engines` requires `>=22.19.0`. The VM's default `node` on `PATH` (`/exec-daemon/node`) is 22.14.0, which is too old for the test suite — ~20 tests fail on it because `node:module` `registerHooks` (used by the TS import shim) only exists in Node 22.15.0+. Run tests/typecheck/live `pi` under the bundled compliant version: `export PATH="$HOME/.nvm/versions/node/v22.22.2/bin:$PATH"` (or `nvm use 22.22.2`). Typecheck and `pi` also work on 22.14.0, but `npm test` does not.
- Test gotcha (ambient git config): 3 files (`test/cursor-cloud-local-state.test.ts`, `test/cursor-provider-cloud-env-validation.test.ts`, `test/cursor-provider-connect-timeout.test.ts`, ~13 tests) fail under the VM's ambient global git config because `~/.gitconfig` injects `url.<token>.insteadof` rewrites for `github.com` SSH/scp URLs, which break the remote-identity probes (`inspectCursorCloudLocalState` intentionally reads user git config). Run the suite with a clean global config to isolate: `HOME=/tmp/clean-home npm test` (create `/tmp/clean-home` once). With Node 22.22.2 + clean `HOME`, the full suite passes (1368 passed, 5 skipped).
...
- Platform limitation (macOS/Windows): the Cursor cloud agent dev environment is **Linux-only**, so the macOS and Windows platform-smoke targets **cannot run here**. `npm run smoke:platform:macos`, `npm run smoke:platform:windows-native`, and the combined `npm run smoke:platform:all` require macOS/Windows hosts or VMs (SSH, Parallels/`prlctl`, rsync, and typically Node 24+), none of which exist in this environment. Run those cross-platform targets on their native hosts / the maintainer's platform harness; in the cloud agent, validate only the Ubuntu/Linux path (unit tests, typecheck, `smoke:visual`, `smoke:local-resume`, and `smoke:platform:doctor` for readiness). Do not treat missing macOS/Windows coverage as a setup failure here — it is an environment limitation.
- TUI screenshots / visual smoke: `npm run smoke:visual -- --label L --prompt "..." --mode agent --out-dir DIR` runs a real Cursor turn in a tmux-hosted pi TUI and writes `.ansi/.txt/.html/.png/.jsonl.path/.manifest.json`. `tmux`, `node-pty`, and `@xterm/xterm` are already present; the PNG step needs the Playwright Chromium browser, installed by the update script via `npx playwright install chromium` (cache at `~/.cache/ms-playwright`). If a PNG capture ever fails with a missing-browser error, re-run `npx playwright install chromium`, or pass `--no-screenshot` and render the `.html` with `agent_browser`.
- Live smoke scripts (`smoke:live`, `smoke:isolated`, `smoke:local-resume*`, `smoke:steering`) resolve `pi` via `command -v pi`, but this repo's `pi` is only at `node_modules/.bin/pi`. Put it on `PATH` first: `export PATH="$HOME/.nvm/versions/node/v22.22.2/bin:$PWD/node_modules/.bin:$PATH"`. `smoke:local-resume` (real local-runtime resume proof) and `smoke:visual` pass in this environment.
- `smoke:live` gotcha: its prereq greps `pi --list-models cursor | tee | rg -q "composer-2.5"` under `set -o pipefail`. The live catalog here is large (~199 rows), so `rg -q` closes the pipe on first match and `pi` dies with SIGPIPE (pipe status 141), failing the prereq even though the model IS listed (`pi --list-models cursor` alone exits 0). Prefer `smoke:local-resume`/`smoke:visual` for real-smoke evidence; this is a script/large-catalog interaction, not a missing dependency.
- `npm run smoke:platform:doctor` reports the Linux cloud agent as expected: the `Host-side visual render probe` (xterm/Playwright), `node-pty self-test`, and `Cursor auth` checks pass, while `macOS SSH`/`Parallels`/`rsync`/`Node 24+` fail because the cross-platform (macOS/Windows) matrix and its VMs are unavailable here. Those failures do not block the local visual/live smokes above.
